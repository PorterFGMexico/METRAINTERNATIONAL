...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
    if( $_POST['form_name'] != '' AND $_POST['form_email'] != '') {
        ...
        $subject = '[INF] Nuevo Mensaje WEB | Forma: Cotizacion';
        ...
        if( $botcheck == '' ) {
			...
			$units = isset($units) ? "Cargo: $units<br><br>" : '';
			$tank = isset($tank) ? "Empresa: $tank<br><br>" : '';
			$liters = isset($liters) ? "Tipo de Servicio: $liters<br><br>" : '';
            $message = isset($message) ? "Mensaje: $message<br><br>" : '';

            $referrer = $_SERVER['HTTP_REFERER'] ? '<br><br><br>Enviado desde: ' . $_SERVER['HTTP_REFERER'] : '';

            $body = "$name $email $phone $units $tank $liters $message $referrer";
